...
__global__ void ForwardCore(Data* polynomial_in, Data* polynomial_out, Root* root_of_unity_table,
                            Modulus* modulus, int shared_index, int logm, // 8 0 6
                            ...
                            bool reduction_poly_check, int mod_count)
{
    ...
    const int block_y = blockIdx.y; // 1
    ...
    int t_2 = N_power - logm - 1; // 14
    location_t offset = 1 << (N_power - logm - 1);
    int t_ = shared_index; // 8
    location_t m = (location_t)1 << logm;


    // 共有64个block，每个block为(8,32). 因此，对于block 0内部，0~7线程先占8个位置，
    // 然后block 1的0~7线程占8个，依次类推，直到64个block的前八个线程用完；此时，占了8*64=2^9个位置；
    // 这个时候就轮到了每个block的8~15线程，以此类推，一直到16383
    ...
    // 计算索引并且填入shared memory，一共是512 * sizeof(Data)
